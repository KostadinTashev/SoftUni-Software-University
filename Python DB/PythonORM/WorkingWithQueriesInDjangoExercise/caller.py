...
def show_highest_rated_art() -> str:
    highest_art = ArtworkGallery.objects.order_by('-rating', 'id').first()
...
def show_the_most_expensive_laptop() -> str:
    expensive_laptop = Laptop.objects.order_by('-price', 'id')
...
def show_hard_dungeons() -> str:
    ...
    result = [
        f"{hd.name} is guarded by {hd.boss_name} who has {hd.boss_health} health points!"
...
def show_workouts() -> str:
    calisthenics_crossfit_workouts = Workout.objects.filter(workout_type__in=['Calisthenics', 'CrossFit'])
    result = [
        f"{ccw.name} from {ccw.workout_type} type has {ccw.difficulty} difficulty!"
        for ccw in calisthenics_crossfit_workouts
    ]
    return '\n'.join(result)
