...
def find_books_by_genre_and_language(book_genre, book_language) -> QuerySet:
...
def filter_authors_by_nationalities(searched_nationality):
    authors = Author.objects.filter(nationality=searched_nationality).order_by('first_name', 'last_name')
    result = [author.biography
